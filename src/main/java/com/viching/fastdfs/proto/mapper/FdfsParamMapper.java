...
public class FdfsParamMapper {
    ...
    /** 对象映射缓存 */
    private static Map<String, com.viching.fastdfs.proto.mapper.ObjectMetaData> mapCache = new HashMap<String, com.viching.fastdfs.proto.mapper.ObjectMetaData>();
    ...
    public static <T> T map(byte[] content, Class<T> genericType, Charset charset) {
        // 获取映射对象
        com.viching.fastdfs.proto.mapper.ObjectMetaData objectMap = getObjectMap(genericType);
     ...
     * @return
     */
    public static <T> com.viching.fastdfs.proto.mapper.ObjectMetaData getObjectMap(Class<T> genericType) {
        if (null == mapCache.get(genericType.getName())) {
            // 还未缓存过
            mapCache.put(genericType.getName(), new com.viching.fastdfs.proto.mapper.ObjectMetaData(genericType));
        }
     ...
     */
    private static <T> T mapByIndex(byte[] content, Class<T> genericType, com.viching.fastdfs.proto.mapper.ObjectMetaData objectMap, Charset charset)
            throws InstantiationException, IllegalAccessException, InvocationTargetException {

        List<com.viching.fastdfs.proto.mapper.FieldMetaData> mappingFields = objectMap.getFieldList();
        T obj = genericType.newInstance();
        for (int i = 0; i < mappingFields.size(); i++) {
            com.viching.fastdfs.proto.mapper.FieldMetaData field = mappingFields.get(i);
            // 设置属性值
    ...
    public static byte[] toByte(Object object, Charset charset) {
        com.viching.fastdfs.proto.mapper.ObjectMetaData objectMap = getObjectMap(object.getClass());
        try {
